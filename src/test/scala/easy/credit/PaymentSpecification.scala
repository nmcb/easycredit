...
object PaymentSpecification:
  ...
  val invalidDateLiterals: TableFor1[Date] =
    Table[Date](
      "valueDate",
      "0000-00-00",
      "0000-13-01",
  ...
  val validRefs: Gen[Ref] =
    for
      ref <- Gen.numStr
    yield
      ERef(ref)

  val validPayments: Gen[Payment] =
    for
      amount    <- validAmounts
      source    <- validIBANs
